<?php
	...
	if($command == "remove")
	{
		...
		if($result = @mysqli_query($link, "DELETE FROM ECO_Reports WHERE ReportIdx = '$reportIdx' ") )
		{
			if($result)
			{
		...
		$projectIdx && $command == "view")
	{
		...
		if($result = @mysqli_query($link, "SELECT ProjectName FROM ECO_Project WHERE ProjectIdx = '$projectIdx'") )
		{
			if( mysqli_result($result,0,0) )
			{
				$selectedProject = mysqli_result($result,0,0);
			}
			mysqli_free_result($result);
		}
		...
		if($result = @mysqli_query($link, "SELECT  Report, NextReport, MarketIdx, SpendTime, SpendPercent,ReportIdx,IsComplete, Ver FROM ECO_Reports
									WHERE ReportIdx = '$reportIdx' 
									AND MarketIdx ='$marketIdx' ".$DateQuery." 
									AND MemberIdx = '$_SESSION[report_login_userIdx]' ") )
		{
		...
		if( $marketIdx )
		{	
			if($result = @mysqli_query($link, "SELECT MarketName, PlatformIdx FROM ECO_Market WHERE MarketIdx = '$marketIdx'") )
			{
				while( $row = mysqli_fetch_array($result) )
				{
		...
		if($selectedMarket)
		{
			if($result = @mysqli_query($link, "SELECT PlatformName FROM ECO_Platform WHERE PlatformIdx = '$PlatformIdx'") )
			{
				while( $row = mysqli_fetch_array($result) )
				{
	...
	if($selectedProject)
	{			
		if($result = @mysqli_query($link, "SELECT IsGame, ProjectIdx FROM ECO_Project WHERE ProjectName = '$selectedProject' "))
		{	
			if( mysqli_result($result,0,0) )
			{
				$disabledStr = "";
			...
			{
				$projectIdx =  mysqli_result($result,0,1);
			}		
			mysqli_free_result($result);
		}
	}	
	...
	if($selectedMarket)
	{
		if($result = @mysqli_query($link, "SELECT MarketIdx FROM ECO_Market WHERE MarketName = '$selectedMarket' "))
		{
			if( mysqli_result($result,0,0) )
				$marketIdx = mysqli_result($result,0,0);
	...
	if($projectIdx && $marketIdx && $command != "addClick" && $command != "editClick")
	{
		...
		if($result = @mysqli_query($link, "SELECT MAX(Ver) FROM ECO_Reports 
									...
									'$reportDateQuery' "))
		{
			if( mysqli_result($result,0,0) )
			{
				$projectVer = mysqli_result($result,0,0);
	...
	if($projectIdx && $spendTime && $spendPercent && $reportDate && $thisWeekReport && $command)
	{
		...
		if($command == "addClick")
		{
			...
			$result = @mysqli_query($link, "INSERT INTO ECO_Reports 
			...
			VALUES('$memberIdx','$reportDate','$projectIdx','$marketIdx','$spendTime',
					'$thisWeekReport','$nextWeekReport','$spendPercent',0,'$projectVer')");
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
		...
		function projectEnterKeyPress(event)
		{
			var frm = document.forms.formSignup;
			//var a = document.getElementById("selectedProjectId").value;

			frm.selectedProject.value = frm.selectedProjectId.value;
			
			if(13 === event.keyCode )
			{
		
				alert(frm.selectedProject.value);
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td colspan="5">
                            <div class="divide">
                                <div class="left">
                                	<input type="text" name="selectedProjectId" list="selectedProject" id="selectedProjectId"  onkeypress="projectEnterKeyPress(event)"
                                	 autofocus required 
                                	<?php
                                		if($selectedProject != "")
                                			echo "value = '$selectedProject'"
                                	?>
                                	>
                                	<datalist id="selectedProject">
									<select id = "selectedProject" name="selectedProject"  autofocus onchange="changeForm()" style="width: 200px" <?php echo $viewSelectStr ?> >
									<option value="">
									<?php
									if($result = mysqli_query($link, "SELECT * FROM ECO_Project order by IsGame,ProjectName asc"))
									{
										...
										{	
											...
											echo "<option value='$row[ProjectName]' selected>";
											else
								...
								<script type="text/javascript">
								          if (!("autofocus" in document.createElement("input"))) {
								            document.getElementById("selectedProjectId").focus();
								          }
								        </script>
                                </div>
                                <div class="right">
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            <div>
							<input type="text" name="spendTime" style="width:25px;"
