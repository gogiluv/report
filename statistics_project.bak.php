<?php
  ...
  {
	$memberNameResult = @mysqli_query($link, "SELECT MemberIdx FROM  ECO_Member WHERE MemberName = '$selectedName' ");
  //$selectedMemberIdx = mysql_result($memberNameResult,0,0);
  $selectedMemberIdx = mysqli_fetch_row($memberNameResult)[0];
	mysqli_free_result($memberNameResult);
  ...
  else  if($_SESSION['report_login_level'] == 1 && $_SESSION['report_login_level'] == 2) //일반 사원  
  {
	...
	{ 
	$memberInfoResult = mysqli_query($link, "SELECT * FROM ECO_Member WHERE MemberName != \"관리자계정\"  
											".$memberIdxQuery."
  ...
  if($year > 2000)
  {
    ...
    $allItems = mysqli_query($link, "SELECT DISTINCT MONTH(Date), ProjectIdx, MarketIdx FROM ECO_Reports 
							WHERE IsComplete=1 
							AND YEAR(DATE)='$year'$month_condition $memberIdxQuery ORDER BY $month_sort ProjectIdx, MarketIdx");

    $totalSpendTimeResult = mysqli_query($link, "SELECT SUM(SpendTime) FROM ECO_Reports 
							WHERE IsComplete=1 
    ...
    $allSpendTimeResult = mysqli_query($link, "SELECT SUM(SpendTime), ProjectIdx, MarketIdx, MONTH(Date) FROM ECO_Reports 
									WHERE IsComplete=1 
									AND YEAR(DATE)='$year' $month_condition  $memberIdxQuery
									GROUP BY ProjectIdx, MarketIdx, MONTH(Date)");
    ...
    <div class="divide">
      <div class="boardtype2">
        <div class="tabcontents" id="tabcontents2">
            ...
            <p class="t2">
              <?php
                if(  $_SESSION['report_login_level'] == 3)
                {
                  ...
                  $memberNameInfo = mysqli_query($link, "SELECT * FROM ECO_Member WHERE MemberName != \"관리자계정\" and Visible = 1");
                    
                      while($row = @mysqli_fetch_array($memberNameInfo))
                  {
                        echo "<option value='$row[MemberName]' ".(($selectedName == $row['MemberName']) ? "selected" : "").">
                    $row[MemberName]</option>\r\n";
                  }
                    
                  @mysqli_free_result($memberInfoResult);
                    echo " </select>";                
                }
              ?>
              년 :
              ...
              <?php
              if($result = @mysqli_query($link, "SELECT DISTINCT SUBSTRING(Date,1,4) FROM ECO_Reports ORDER BY Date DESC")) {
          ...
          {
            #$count = mysqli_num_rows($allItems);
