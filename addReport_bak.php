<?php
		...
		$projectIdx && $command == "view")
	{
		...
		if($result = @mysqli_query($link, "SELECT ProjectName FROM ECO_Project WHERE ProjectIdx = '$projectIdx'") )
		{
			...
			*/
			//20190319 kbo
			if(!empty($result)){$selectedProject = mysqli_fetch_row($result)[0];}
			mysqli_free_result($result);
		}
    ...
    */
    //20190319 kbo
    $result = mysqli_query($link, "SELECT IsGame, ProjectIdx FROM ECO_Project WHERE ProjectName = '$selectedProject' ");
	...
	if($selectedMarket)
	{
		if($result = @mysqli_query($link, "SELECT MarketIdx FROM ECO_Market WHERE MarketName = '$selectedMarket' "))
		{
			...
			*/
			//20190319 kbo
			if($result->num_rows>0){
				$marketIdx = mysqli_fetch_row($result)[0];
			}
	...
	if($projectIdx && $marketIdx && $command != "addClick" && $command != "editClick")
	{
		...
		*/
		// 20190319 kbo
		$result = mysqli_query($link, "SELECT MAX(Ver) FROM ECO_Reports 
								            WHERE ProjectIdx = '$projectIdx' 
								            AND MarketIdx = '$marketIdx'
                            '$reportDateQuery' ");
    if($result->num_rows>0){
      $projectVer = mysqli_fetch_row($result)[0];
    } else {
      $projectVer = "000";
    }		
	}
	...
	if($projectIdx && $spendTime && $spendPercent && $reportDate && $thisWeekReport && $command)
	{
		...
		if($command == "addClick")
		{
			...
			(MemberIdx,work_d,ProjectIdx,MarketIdx,work_h,
