<?php
	...
	{
		$memberInforResult = mysql_query("SELECT * FROM ECO_Member WHERE MemberName != \"관리자계정\"  
	...
	else	//1 팀원
	{
	 ...
	 $allItems = mysql_query("SELECT DISTINCT MONTH(Date), ProjectIdx, MarketIdx FROM ECO_Reports 
							WHERE IsComplete=1 AND YEAR(DATE)='$year'$month_condition ORDER BY $month_sort ProjectIdx, MarketIdx", $link);
            ...
            <div class="boardtype2">
                ...
                <div class="tabcontents" id="tabcontents2">
                    ...
                    <table cellpadding="0" cellspacing="0" width="520" border="0">
                      ...
                      <select>
						<?php
							...
							else
							{
								...
								{
										while($row = mysql_fecth_array($memberInforResult) )
										{
                     ...
                     <select>
						<?php
							$selectedYearResult = mysql_query("SELECT DISTINCT YEAR(Date) FROM ECO_Reports ORDER BY Date DESC",$link);

							if($selectedYearResult)
							{
								while($row = mysql_fetch_array($selectedYearResult))
								{
									echo "<option> $row[0] </option>";				
								}
								@mysql_free_result($selectedYearResult);
							}
						?>
					</select>
                        월 : 
