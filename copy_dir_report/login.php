<?php
	...
	if($id && $pw) {
		if($result = @mysql_query("SELECT * FROM ECO_Member WHERE MemberIdx='$id' LIMIT 1", $link)) {
...
<head runat="server">
    ...
    function check_form(obj) {
		...
		var strid = obj.member_name.value;
        ...
        <div class="C2Slogin">
            ...
            <fieldset class="loginarea1"  style="height:66px" >
                <div class="field">
					...
					<?php
						if($result = @mysql_query("SELECT * FROM ECO_Member WHERE Visible = 1 ORDER BY EN ASC", $link)) {
							while($row = @mysql_fetch_array($result)) {
								echo "<option value='$row[MemberIdx]'".
								(($id == $row[MemberName]) ? "selected" : "").">$row[MemberName]</option>\r\n";
							}
							@mysql_free_result($result);
						}
					?>
