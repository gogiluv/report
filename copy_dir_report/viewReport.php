<?php
	...
	if($reportIdx)
	{
		$result = @mysql_query("SELECT * FROM ECO_Reports WHERE ReportIdx = '$reportIdx'",$link);
		...
		while($row = mysql_fetch_array($result))
		{
			...
			$meberNameResult = @mysql_query("SELECT MemberName FROM ECO_Member WHERE MemberIdx = '$row[MemberIdx]'",$link);
			$projectNameResult = @mysql_query("SELECT ProjectName FROM ECO_Project WHERE ProjectIdx = '$row[ProjectIdx]'",$link);
			$marketNameResult = @mysql_query("SELECT MarketName FROM ECO_Market WHERE MarketIdx = '$row[MarketIdx]'",$link);
			$platformNameResult = @mysql_query("SELECT PlatformName FROM ECO_Platform
														WHERE PlatformIdx =(SELECT PlatformIdx FROM ECO_Market 
														WHERE MarketIdx = '$row[MarketIdx]')",$link);

			$memberName = @mysql_result($meberNameResult,0,0);
			$projectName = @mysql_result($projectNameResult,0,0);
			$marketName = @mysql_result($marketNameResult,0,0);
			$platformName = @mysql_result($platformNameResult,0,0);
			...
			@mysql_free_result($meberNameResult);
			@mysql_free_result($projectNameResult);
			@mysql_free_result($marketNameResult);
			@mysql_free_result($platformNameResult);
